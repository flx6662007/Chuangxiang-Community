...
第一阶段计划开发公开赛事列表。游客可查看公开赛事，草稿、下架内容和内部管理字段不应公开；同一赛事不同届次需要区分。持久化字段基线已整理为 [赛事字段表](competition-fields-table.md)，日期精度、来源及发布含义见 [赛事字段说明](competition-fields.md)；当前仍没有可调用的赛事接口。以下 HTTP 契约尚需随实现对齐：
