...
版本：v1.1；2026-09-26，补充登录用户的举报和申诉权限。用户字段沿用首版建模基线，不为治理入口增加重复的账号状态列。完整约束、权限及流程见[用户字段说明](database-fields.md#accounts)，当前运行与交付状态见[团队进度](progress.md)。
...
外键的数据库列名由 Django 自动追加 `_id`。有效限制同时禁止新增招募和提交新申请，保留登录、已有记录、退出、举报与申诉的能力及已有成员关系。举报和申诉只要求当前登录，不要求邮箱核验或联系方式。有效条件：当前时间处于开始与结束之间且未提前解除；到期自然恢复。重复处理同一有效限制不叠加或重置时长。

本人可针对 `UserRestriction` 创建 `governance.Appeal`，由原限制创建人之外的管理员复核；申诉结论不直接修改 `revoked_at/revoked_by/revoke_reason`。实际提前解除仍须另行执行。举报与申诉记录通过外键关联用户，字段和限流规则见[治理字段](database-fields.md#governance)及[接口说明](api-governance.md)，不在 User 中保存申诉数组或独立申诉权限标记。
...
| 未验证账号 | 可登录、查看公共内容及账号状态、重发验证码、提交举报与申诉；不可发布招募或提交申请 |
