...
import { csrfFromCookie, isFinishedPasswordReset, isPendingPasswordReset, isSignedOut, withProfileSessionCheck } from '../utils/account'

const auth = useCsrf(axios.create({
...
const readSession = () => auth.get('/auth/session', { validateStatus: sessionStatus })
...
export async function getProfile() {
  return withProfileSessionCheck(async () => (await http.get('/accounts/me/')).data, readSession)
}

export async function getSessionProfile() {
  const response = await readSession()
  if (response.status === 401 && isSignedOut(response.data)) return null
  if (response.status !== 200 || response.data?.meta?.is_authenticated !== true) unexpectedResponse(response)
  return getProfile()
}

export async function signIn(email, password, register = false) {
  await auth.post(register ? '/auth/signup' : '/auth/login', { email, password })
  return getProfile()
}

export async function signOut() {
  const response = await auth.delete('/auth/session', { validateStatus: sessionStatus })
  if (!isSignedOut(response.data)) unexpectedResponse(response)
}

export async function saveContacts(contacts) {
  return withProfileSessionCheck(async () => (await http.patch('/accounts/me/', contacts)).data, readSession)
}
...
export async function requestPasswordReset(email) {
  const response = await auth.post('/auth/password/request', { email }, { validateStatus: sessionStatus })
  if (!isPendingPasswordReset(response.data)) unexpectedResponse(response)
}

export async function resetPassword(key, password) {
  const response = await auth.post('/auth/password/reset', { key, password }, { validateStatus: sessionStatus })
